...
internal static class VibratoGenerator
{
    ...
    public static double GetValue(int dvb, int i) => Values[(dvb * 8) + ExtractBits((uint)i, 10, 0x1C00)];

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static uint ExtractBits(uint value, byte start, uint mask) => (value & mask) >> start;
}
